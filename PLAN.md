...
`choices/varying-choice` and `choices/sticky-choice` with the imported legacy
suites still feature-gated (`2026-04-23 04:26`).
...
`choices/divert-choice`, `choices/mixed-choice`, `choices/varying-choice`,
and `choices/sticky-choice` are green again; the imported legacy
compiler-conformance and csharp suites are retained behind the separate legacy
features so they can keep migrating without blocking the default workspace run
(`2026-04-23 04:26`).
...
- The imported legacy compiler-to-runtime suite now has tokenizer-level
  ...
  `choices/multi-choice`, `choices/suppress-choice`, `choices/label-scope`,
  `choices/divert-choice`, `choices/mixed-choice`, `choices/varying-choice`,
  and `choices/sticky-choice`). The checklist queue has been reordered to
  follow the broad syntax progression in `WritingWithInk.md`, so the next
  compiler-conformance work starts from the earliest dependency-light syntax
  slice and now moves on to `choices/fallback-choice`.
