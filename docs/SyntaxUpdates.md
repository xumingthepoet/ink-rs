...
This file records intentional language and semantic changes made by ink-rs.
ink-rs is a new domain-specific language for narrative games, implemented in
Rust and inspired by ink by inkle. Mentions of Ink in this changelog are
historical background, not a compatibility contract. This file is not the
fastest way to learn current syntax.
...
teach only the current language syntax. Keep historical details in this file.
...
- status: supported
- background: Ink treats leading braced expressions on choice
...
- author impact: replace condition-style choice prefixes written as
...
- status: supported
- background: Ink can print values in text interpolation, but
...
- author impact: replace local numeric/stringification helpers with
...
- status: supported
- background: Ink does not have a built-in source form that
...
- status: supported
- background: Ink does not have typed array or `Dict<K, V>`
...
- rationale: typed arrays and Dicts need a concise source-level iteration form
  ...
  JSON format changes.
- author impact: replace hand-written counter loops and recursive helper
...
- status: supported
- background: Ink lowers `and` / `&&` and `or` / `||` as eager
...
- status: supported
- background: Ink does not have typed dynamic arrays or typed
...
- author impact: replace slot-fill patterns such as `items[count] = value`
...
- status: supported
- background: Ink does not have explicit generic `Dict<K, V>`
...
- documentation effect: `SyntaxReference.md` documents the new Dict helpers.
- rationale: registry and table-shaped story state should not require parallel
  ...
  entries.
- author impact: use `DICT_SIZE(dict)` instead of trying `LEN(dict)`, use
...
- status: supported
- background: conventional arithmetic treats multiplication, division,
...
- rationale: the previous parser table gave each multiplicative operator a
  ...
  evaluate as `a * (b / c)`.
- author impact: remove defensive parentheses that only worked around the
  previous parser bug. Keep parentheses when a non-left-associative grouping is
  intended.
- tests: parser snapshots cover mixed multiplicative chains, and the
  ...
  formulas whose outputs differ under the previous precedence table.

## 2026-05-20: Elixir-Style Struct And Dict Literals

- status: supported
- background: Ink does not have typed `STRUCT` or `Dict<K, V>`
...
- ink-rs behavior: struct literals now use an explicit typed form such as
  ...
  unsupported and report current-syntax diagnostics.
...
- rationale: `%Type{...}` removes ambiguity between struct values and braced ink
  ...
  format.
- author impact: replace struct literal expressions with `%Type{...}` and
...
- tests: parser, diagnostics, analysis, lowering, typed fixtures, runtime API,
  ...
  unsupported syntax diagnostics, mixed Dict key diagnostics, modulo parsing, and
...
- author impact: use `Dict<string, V>` or `Dict<int, V>` instead of
...
- status: supported
- background: Ink does not have explicit module interfaces,
...
- author impact: use `interface<IItem>` variables and explicit
...
- status: supported
- background: Ink uses LIST declarations for symbolic value
...
- rationale: story code needs named state sets without depending on removed
  Ink list behavior or integer/string constants that weaken type checks.
- author impact: replace string or integer state constants with an enum
...
- status: supported
- background: Ink exposes `EvaluateFunction` as a host API but
...
- author impact: keep ordinary `function` declarations for ink-only
...
- status: removed
- background: Ink uses `+` as a sticky/repeatable choice
...
- rationale: once-only choice behavior was already removed, so keeping a second
  ...
  distinct semantics.
- author impact: replace every leading choice marker `+` with `*`, keeping
  the same indentation, condition, text, and divert body.
- tests: compiler parser tests reject `+` choice markers, runtime choice
  fixtures use only `*`, and the unsupported-sequence diagnostic fixture still
...
- background: Ink uses source includes rather than explicit
...
- ink-rs behavior: module imports now use leading `FROM`. `FROM module IMPORT
  ...
  authorize static `module::symbol` access. `IMPORT name FROM module` is not
  current syntax and reports an unsupported import diagnostic.
...
- author impact: replace `IMPORT target FROM left` with
...
- rationale: large modules can expose enough symbols that one-line import lists
  become hard to read.
- author impact: replace block imports with a comma-separated
  `FROM module IMPORT ...` line.
...
- status: removed
- background: Ink and earlier ink-rs accepted multiline control
...
- ink-rs behavior: multiline control blocks now require an explicit control
  ...
  switch comparisons. Keywordless multiline forms are removed rather than kept
  as aliases.
...
- author impact: add `if` after the opening brace for boolean multiline
...
- status: supported
- background: Ink supports switch-style conditionals such as
...
- author impact: use switch syntax for enumerated selector values. Add
...
- status: supported
- background: Ink examples use root-level knots declared with
...
- author impact: wrap runnable examples in an explicit module, rename
...
- status: removed
- background: Ink supports source-level alternatives inside
...
- ink-rs behavior: source sequences, cycles, shuffles, and once-only
  ...
  an unsupported syntax diagnostic instead of lowering them to visit-count-based
...
- author impact: model progression explicitly with typed variables and
...
- tests: compiler parser/lowering unit tests, `diagnostics/unsupported-sequence.ink`,
...
- background: Ink treats `*` choices as once-only by default
...
- documentation effect: `SyntaxReference.md` describes repeatable choices,
  explicit
...
- rationale: implicit once-only choices depended on removed visit-count state.
  Keeping the tutorial and unused flags made the current repeatable choice
  model look inconsistent.
- author impact: replace reliance on implicit `*` disappearance with a
...
- status: supported
- background: leading braced expressions on choice lines are parsed as
...
- author impact: replace `* {condition}<>{dynamic_text}` or
...
- background: Ink permits root-level story content, top-level
...
- author impact: wrap runnable content in a module with `== main ==`,
...
- tests: `docs_module_import_example_runs`,
  ...
  module root-behavior diagnostics in the behavior-focused files under
  `crates/ink-test/tests/`, plus module parser, analysis, lowering, compiler
  API, fixture, language-change, and runtime-loading tests.

## 2026-04-28: Compiler Entry Requires Explicit Module Headers

- status: supported/removed
- background: Ink accepts root-level story content and top-level
...
- rationale: accepting root stories in production kept a stale path alive after
  maintained fixtures had moved to module syntax, making tests less faithful to
  the current language.
- author impact: move root-level story content into a knot or stitch
...
- status: supported
- background: Ink uses divert target values in flow APIs and
...
- ink-rs behavior: divert targets are first-class typed values written as `->`.
  ...
  `== function pick() => -> ==` and `EXTERNAL pick() => ->`. The unsupported
  typed return marker `->` is rejected with a diagnostic. The historical Ink parameter
...
- author impact: rewrite typed function and external returns from
...
- status: removed/supported
- background: Ink allows implicit variable divert targets such
...
- ink-rs behavior: static diverts remain `-> knot.path`. Dynamic diverts must
  ...
  `-> {pick(flag)}(arg)`, and the expression must type-check as `->`.
  Variable-rooted direct diverts produce an unsupported syntax diagnostic. Choice line
...
- documentation effect: `SyntaxReference.md` describes explicit dynamic
  ...
  count-related runtime tokens as historical compiled-story compatibility rather
...
- author impact: rewrite `-> next` to `-> {next}` when `next` is a
...
- tests: dynamic divert parser, analysis, lowering, choice, save/load, removed
  count diagnostics, language-change coverage, compiler conformance,
  runtime unit, and language integration tests updated with this change.

## 2026-04-26: CONST Declarations Require Explicit Types

- status: supported
- background: Ink constants are dynamically typed and declared
...
- documentation effect: `SyntaxReference.md` updates constants examples to use
  explicit types.
...
- author impact: rewrite `CONST NAME = value` as `CONST NAME: Type =
  value`.
...
- status: removed
- background: Ink allows global variables to be introduced with
...
- documentation effect: `SyntaxReference.md` records the scope
  restriction in "historical changelog" and in the global variable
  declaration section.
...
- author impact: move nested `VAR` declarations to module top level. If
...
- tests: `nested_global_var_declarations_report_current_syntax_error` in
  ...
  nested `VAR` parser diagnostics in `crates/ink-compiler/src/syntax/parser.rs`,
  and
...
- status: supported
- background: Ink variables, temporary variables, function
...
- ink-rs behavior: direct self tail recursion in the shape
  ...
  jump back to the function body, avoiding growth of the runtime function
  callstack for that recursive step. Mutual recursion and non-tail recursion
  keep normal call behavior.
- documentation effect: `SyntaxReference.md` must use typed declarations, add
  struct and array syntax, document typed functions and externals, and document
  the array builtins and tail-call behavior as ink-rs language extensions.
...
- author impact: add explicit type annotations to all `VAR` and `temp`
...
- status: removed
- background: Ink supports `LIST` declarations for named list
  origins and list items, documented in the Ink "Advanced State Tracking"
...
- ink-rs behavior: `LIST` declarations produce an unsupported syntax diagnostic.
  Compiled story JSON no longer serializes `listDefs`, list value objects, or
  list-specific runtime tokens.
- documentation effect: `SyntaxReference.md` omits `LIST` declarations from the
  current language reference and records the change in "historical changelog".
...
- author impact: use variables, functions, or host-side data for inventory
  and set-like game state until a replacement list design is added.
- tests: `unsupported_list_declaration_reports_unsupported_syntax_diagnostic` in
  the behavior-focused files under `crates/ink-test/tests/`.
