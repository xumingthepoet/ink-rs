...
4. Read the matching Rust runtime implementation in `ink-runtime/lib/src`.
...
- Find the Rust runtime type or JSON reader behavior in `ink-runtime`.
- Prefer generating JSON that the Rust runtime already accepts.
- Avoid modifying `ink-runtime` unless no compiler-side adapter can work.
...
- Runtime smoke test through `ink_runtime`.
