...
turn indices, or earlier save-state versions. Saving is allowed only while
visible choices are pending. The serialized state is the stable execution
snapshot from immediately before choice generation; generated choices are
regenerated by continuing after load. Ordinary non-choice states are rejected
as save points. Earlier saves and v3 saves that contain removed fields such as
