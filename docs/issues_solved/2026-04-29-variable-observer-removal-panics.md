...
Resolution: Removed the variable observer runtime API, observer batching state, observer-specific test helper APIs, and the dedicated observer fixture. Updated current runtime documentation to describe host integration without variable observers while keeping direct variable get/set support.
Validation: `cargo check -p ink-runtime -p ink-test`; `cargo test -p ink-runtime`; `cargo test -p ink-test --test runtime_api`; `make gate TIMEOUT='f(){ shift; "$$@"; }; f'`.
