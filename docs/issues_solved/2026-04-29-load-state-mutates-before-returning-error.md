...
Resolution: Changed save-state loading to parse the flow, variables, seed, and previous random into temporary values first, then commit them to `StoryState` only after all validation succeeds. Also converted variable-state downcast failures to `BadJson` so validation errors do not panic while loading into the temporary state.
Validation:
- `cargo fmt --all --check`
- `cargo check -p ink-runtime`
- `cargo test -p ink-runtime failed_load_state_keeps_existing_story_state`
