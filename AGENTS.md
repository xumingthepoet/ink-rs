...
- [2026-04-25 00:00 CST] [👍8][👎0] The C# tests campaign queue is the ignored non-LIST tests in `crates/ink-test/tests/csharp_tests/mod.rs`. First remove the ignore marker for the next test in file order and run it focused. If it already passes under existing generic behavior, enable it, validate, and commit without adding compiler churn.
...
- [2026-04-25 00:35 CST] [👍9][👎0] Diagnostics belong over parsed-model semantics: parse recoverable C# forms such as function stitches, empty divert/thread statements, empty choices, blank weave-style choices, and suspicious text into spanned nodes, then report parser/analysis diagnostics. Sealed conditional/sequence choices with loose ends are analysis errors. Focused warning tests should assert the intended warning, not any later warning.
