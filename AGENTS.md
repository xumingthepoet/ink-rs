...
- [2026-04-24 07:47 CST] [👍0][👎0] Choice labels are compile-time symbols for read counts. Parse `(label)` into `Choice.identifier`, parse `{label}` as a variable reference, then lower it to `CNT?` only after resolving the label to the generated choice container path in the current weave scope.
