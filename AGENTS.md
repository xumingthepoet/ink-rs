...
- [2026-04-25 00:00 CST] [👍1][👎0] The C# tests campaign queue is the ignored non-LIST tests in `crates/ink-test/tests/csharp_tests/mod.rs`. First remove the ignore marker for the next test in file order, run it focused, then fix behavior until it passes; do not add new ignores to hide failures.
...
- [2026-04-25 00:35 CST] [👍0][👎0] Flow diagnostics belong in compiler analysis over the parsed model. Preserve source spans on parsed nodes such as `Return`, ignore story-scope declarations for termination, and model branch/sequence termination structurally instead of relying on emitted runtime JSON or fixture strings.
