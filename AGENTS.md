...
- [2026-04-25 00:35 CST] [👍10][👎0] Diagnostics belong over parsed-model/compiler-pipeline semantics: parse recoverable C# forms into spanned nodes, then report parser/analysis diagnostics. Story-wide `CONST` redefinition only errors when the expression value changes. Sealed conditional/sequence choices with loose ends are analysis errors. `INCLUDE` inserts non-flow content at the include site and appends included flows to story end.
