...
- [2026-04-24 00:02 CST] [👍38][👎0] First distinguish “queue maintenance” from real compiler work: if a priority fixture already passes under existing generic behavior, just enable it, validate it, update `priority.md`, and commit. Only change compiler code when the fixture exposes a genuine model or export gap.
...
- [2026-04-24 08:50 CST] [👍4][👎0] Function calls and string expressions are expressions: lower call arguments before the runtime command/user function token. Divert-target arguments used as values count visits and turns unless a direct `TURNS_SINCE`/`READ_COUNT` call gives a narrower purpose. Lower string expressions as `str ... /str` with nested `{...}` emitting `ev ... out /ev`.
