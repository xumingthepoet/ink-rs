...
- [2026-04-24 08:38 CST] [👍2][👎0] Expression operators should be parsed into `Expression::Binary` and lowered by emitting operands followed by the runtime native function token. Split only at top-level operators, respecting quoted strings, so text literals do not accidentally drive expression structure.
