...
- [2026-04-24 08:26 CST] [👍1][👎0] Follow C# `CompactPathString` when emitting runtime paths: compute the valid relative path, but only serialize it when it is shorter than the global path. Choice point targets, choice-body diverts, and return labels may therefore choose different path forms inside the same flow.
...
- [2026-04-24 08:31 CST] [👍2][👎0] Braced inline content is not always a sequence. If the braces contain no sequence separator and parse as an expression, keep it as expression content and lower text output as `ev ... out /ev`; this supports dynamic text and tags without treating variable references as one-element sequences.
- [2026-04-24 08:35 CST] [👍1][👎0] A `~ name = expr` line is variable reassignment, not a temp assignment. Lower the expression inside `ev ... /ev`, then emit `VAR=` with `re:true` outside evaluation; reserve `temp=` for actual temporary variables and compiler-generated temporaries.
- [2026-04-24 08:43 CST] [👍0][👎0] When lowering nested `ContentList` objects, append directly into the parent runtime content vector when path-sensitive children may be created. Lowering into a temporary vector and then extending loses the final container index, which breaks compact paths for embedded sequences and dynamic tags.
...
- [2026-04-24 08:38 CST] [👍1][👎0] Expression operators should be parsed into `Expression::Binary` and lowered by emitting operands followed by the runtime native function token. Split only at top-level operators, respecting quoted strings, so text literals do not accidentally drive expression structure.
- [2026-04-24 08:17 CST] [👍2][👎0] Top-level `VAR` declarations lower into the root named `global decl` container, not the main weave. Tag-only lines emit tag delimiters and text without adding a newline. Flow auto-divert to the first child stitch only applies when the parent flow content does not already end in a terminator.
