...
- [2026-04-25 00:35 CST] [👍13][👎0] Diagnostics and compiler-pipeline behavior belong over parsed-model semantics. Story-wide `CONST` redefinition only errors when values change; constant refs must expand during expression lowering before path indexes are planned, not as post-lowering object rewrites. Sealed conditional/sequence loose choices are analysis errors. `TODO:` is an AuthorWarning node and is ignored by loose-end termination. `INCLUDE` inserts non-flow content at the include site and appends included flows to story end.
