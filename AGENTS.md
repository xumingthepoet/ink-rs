...
- [2026-04-25 00:35 CST] [👍10][👎0] Diagnostics belong over parsed-model semantics: parse recoverable C# forms such as function stitches, empty divert/thread statements, blank weave-style choices, and suspicious text into spanned nodes, then report parser/analysis diagnostics. Story-wide `CONST` redefinition only errors when the expression value changes. Sealed conditional/sequence choices with loose ends are analysis errors.
