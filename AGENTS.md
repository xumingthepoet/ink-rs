...
- [2026-04-24 00:02 CST] [👍22][👎0] First distinguish “queue maintenance” from real compiler work: if a priority fixture already passes under existing generic behavior, just enable it, validate it, update `priority.md`, and commit. Only change compiler code when the fixture exposes a genuine model or export gap.
...
- [2026-04-24 09:34 CST] [👍7][👎0] Weave lowering needs a current runtime-container model. After gathers, content and choices stay in that gather; inside flows, linear weave objects must carry flow/stitch container paths. Branch rejoin diverts should use C#-style compact paths, choosing relative only when shorter than absolute.
...
- [2026-04-24 09:06 CST] [👍1][👎0] Preserve C# trial order when extending syntax. Braced logic should try explicit sequence annotations before conditional expressions, and divert targets before unary prefix operators, so syntax markers are not consumed as ordinary expression text. Add focused regressions around nearby ambiguous forms.
