...
- [2026-04-25 00:35 CST] [👍3][👎0] Flow diagnostics belong in compiler analysis over the parsed model. Parse invalid-but-diagnosable C# forms such as function stitches and empty divert statements, preserve spans on nodes such as `Return`, use a flow-symbol index for target restrictions, and avoid relying on emitted runtime JSON or fixture strings.
- [2026-04-24 09:06 CST] [👍6][👎0] Preserve C# trial order and type shape when extending syntax. Braced logic tries explicit sequence annotations before conditionals; sequence annotations are combinable flags like `shuffle once`, not separate cases. Divert targets must be recognized before unary prefix operators. Identifier predicates should be shared and mirror C# character ranges. Choice conditions may continue across newlines into indented start content. Dash-brace inside conditionals is nested content, not a branch condition. If shuffle output differs while JSON shape is right, verify runtime `System.Random` and `previousRandom` semantics first.
