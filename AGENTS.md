...
- [2026-04-24 08:35 CST] [👍0][👎0] A `~ name = expr` line is variable reassignment, not a temp assignment. Lower the expression inside `ev ... /ev`, then emit `VAR=` with `re:true` outside evaluation; reserve `temp=` for actual temporary variables and compiler-generated temporaries.
