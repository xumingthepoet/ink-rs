...
- [2026-04-24 07:33 CST] [👍0][👎0] Nested weave grouping is depth-delimited, not type-delimited. Once a deeper choice/gather opens a nested weave, keep following ordinary content inside it until a choice/gather at the current or outer depth appears; otherwise gather text and later choices escape to the wrong container.
- [2026-04-24 07:38 CST] [👍0][👎0] C# gather auto-enter depends on whether the previous section had choices. A gather following choice content becomes named content only; a gather following a no-choice gather section is inserted into the previous gather container, preserving chained gather paths and terminal `done` containers.
- [2026-04-24 07:47 CST] [👍1][👎0] Choice labels are compile-time symbols for read counts. Parse `(label)` into `Choice.identifier`, parse `{label}` as a variable reference, then lower it to `CNT?` only after resolving the label to the generated choice container path in the current weave scope.
- [2026-04-24 08:06 CST] [👍2][👎0] Cross-flow weave-point references need a story-level label index keyed by both unqualified labels and flow-qualified aliases. Still let the current weave's local labels win during lowering, so same-weave `CNT?` paths stay relative while sibling or dotted references resolve to stable absolute paths.
- [2026-04-24 08:12 CST] [👍0][👎0] Build the current weave's local label table before lowering any choice bodies. Labels can be referenced before their defining choice/gather and can be divert targets as well as read-count operands, so sequential insertion during lowering misses valid forward references.
- [2026-04-24 08:17 CST] [👍0][👎0] Top-level `VAR` declarations lower into the root named `global decl` container, not the main weave. Tag-only lines emit tag delimiters and text without adding a newline. Flow auto-divert to the first child stitch only applies when the parent flow content does not already end in a terminator.
