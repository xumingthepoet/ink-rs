...
- [2026-04-24 09:06 CST] [👍7][👎0] Preserve C# trial order/type shape in syntax. Braced logic tries sequence annotations, then conditionals, then expression/default sequence. Divert targets parse before unary prefix. Choice conditions can continue across newline; dash-brace in conditional branches is nested content. Choice parsing separates visible content before trailing diverts; divert-only choices are invisible defaults even with targets. For shuffle output, check C# random semantics first.
