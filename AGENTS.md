...
- [2026-04-25 00:00 CST] [👍4][👎0] The C# tests campaign queue is the ignored non-LIST tests in `crates/ink-test/tests/csharp_tests/mod.rs`. First remove the ignore marker for the next test in file order and run it focused. If failure sits in ported Rust scaffolding, compare upstream C# API overload defaults and parser sentinels like `ParseSuccess` before changing compiler behavior.
...
- [2026-04-24 09:06 CST] [👍3][👎0] Preserve C# trial order and type shape when extending syntax. Braced logic tries explicit sequence annotations before conditionals; sequence annotations are combinable flags like `shuffle once`, not separate cases. Divert targets must be recognized before unary prefix operators. Identifier predicates should be shared and mirror C# character ranges.
