...
- [2026-04-24 00:02 CST] [👍7][👎0] First distinguish “queue maintenance” from real compiler work: if a priority fixture already passes under existing generic behavior, just enable it, validate it, update `priority.md`, and commit. Only change compiler code when the fixture exposes a genuine model or export gap.
- [2026-04-24 00:22 CST] [👍0][👎0] When a conformance fixture introduces a new structural node in the parse snapshot, add that node to the parsed model first, then lower JSON from that model. Keeping `Story`/`Flow` aligned with the C# compiler shape prevents flow semantics from leaking into ad hoc export logic and keeps parser, snapshot, and runtime output moving together.
