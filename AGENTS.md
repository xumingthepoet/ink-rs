...
- [2026-04-24 08:50 CST] [👍2][👎0] Function calls and string expressions are expressions: lower call arguments before the runtime command/user function token. Divert-target arguments used as values count visits and turns unless a direct `TURNS_SINCE`/`READ_COUNT` call gives a narrower purpose. Lower string expressions as `str ... /str` with nested `{...}` emitting `ev ... out /ev`.
