...
- [2026-04-24 09:06 CST] [👍4][👎0] Preserve C# trial order and type shape when extending syntax. Braced logic tries explicit sequence annotations before conditionals; sequence annotations are combinable flags like `shuffle once`, not separate cases. Divert targets must be recognized before unary prefix operators. Identifier predicates should be shared and mirror C# character ranges. If shuffle output differs while JSON shape is right, verify runtime `System.Random` and `previousRandom` semantics first.
