...
- [2026-04-25 00:00 CST] [👍3][👎0] The C# tests campaign queue is the ignored non-LIST tests in `crates/ink-test/tests/csharp_tests/mod.rs`. First remove the ignore marker for the next test in file order and run it focused. If failure sits in ported Rust scaffolding, compare upstream C# API overload defaults and parser sentinels like `ParseSuccess` before changing compiler behavior.
