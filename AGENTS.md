...
- [2026-04-25 00:00 CST] [👍6][👎0] The C# tests campaign queue is the ignored non-LIST tests in `crates/ink-test/tests/csharp_tests/mod.rs`. First remove the ignore marker for the next test in file order and run it focused. If it already passes under existing generic behavior, enable it, validate, and commit without adding compiler churn.
...
- [2026-04-25 00:35 CST] [👍6][👎0] Diagnostics belong over parsed-model semantics. Parse invalid-or-warning C# forms such as function stitches, empty divert statements, empty choices, and suspicious text into nodes with spans, then report diagnostics. Focused C# warning tests should assert the intended warning, not just any later analysis warning.
