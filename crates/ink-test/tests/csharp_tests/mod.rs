...
impl CSharpHarness {
    ...
    pub fn compile_string(
        ...
        let runtime_handler: Rc<RefCell<dyn ErrorHandler>> = Rc::new(RefCell::new(
            RuntimeErrorHandler {
...
-> END"#,
    ...
    fn TestCommentEliminator() {
        ...
        let processed = CommentEliminator::process(test_content.to_string())
            .unwrap();
        let expected = "A\nA  A\n\nA * A * / A\n\n\n";
...
"#,
    ...
    fn TestReturnTextWarning() {
        let mut parser = ink_compiler::parser::InkParser::new("== test ==\n return something", None, None);
        let result = parser.parse();
        assert!(
            result
                .diagnostics
...
"#,
            ...
            assert_eq!(
                0,
                story.get_visit_count_at_path_string("TestKnot").expect("expected visit count")
            );
            assert_eq!(
                0,
                story.get_visit_count_at_path_string("TestKnot2").expect("expected visit count")
            );
