...
fn auto_stitch_test() -> Result<(), StoryError> {
    let json_string = common::get_json_string("inkfiles/stitch/auto-stitch.ink.json");
...
fn manual_stitch_test() -> Result<(), StoryError> {
    let json_string = common::get_json_string("inkfiles/stitch/manual-stitch.ink.json");
    let mut story = Story::new(&json_string);
    let mut text: Vec<String> = Vec::new();
