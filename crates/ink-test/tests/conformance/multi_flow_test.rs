...
fn basics_test() -> Result<(), StoryError> {
    let json_string = common::get_json_string("inkfiles/runtime/multiflow-basics.ink.json");
...
fn multiflow_save_load_threads() -> Result<(), StoryError> {
    let json_string =
        common::get_json_string("inkfiles/runtime/multiflow-saveloadthreads.ink.json");
    let mut story = Story::new(&json_string);
